...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		// Garante que curWacky sempre seja um array com pelo menos 2 strings (mesmo que vazias)
		var candidate = FlxG.random.getObject(getIntroTextShit());
		if (candidate == null) {
			curWacky = ["", ""];
		} else {
			// Se veio só 1 linha, garante 2 elementos
			if (candidate.length == 1) candidate.push("");
			curWacky = candidate;
		}
		...
		if(!closedState)
		{
			...
			switch (sickBeats)
			{
				...
				case 9:
					...
					ngSpr.visible = false;
				case 10:
					if (curWacky != null && curWacky.length > 0 && curWacky[0] != null && curWacky[0].length > 0)
						createCoolText([curWacky[0]]);
					else
						createCoolText(["ZorksBR"]); // fallback vazio ou algo como ["Your Title"]
				case 12:
					if (curWacky != null && curWacky.length > 1 && curWacky[1] != null && curWacky[1].length > 0)
